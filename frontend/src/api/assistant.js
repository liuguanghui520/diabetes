...
async function readSseStream(response, handlers = {}) {
    ...
    if (!reader) {
        throw new Error('ReadableStream is not available on response.body')
    }
    ...
    function emitEvent() {
        ...
        if (eventName === 'message') {
            const delta = parsed?.delta ?? parsed?.answer ?? parsed?.content ?? rawData

            if (delta) {
                content += delta
                handlers.onDelta?.(delta, parsed)
            }
            ...
            handlers.onError?.(parsed || rawData)
        }
...
function sendMockAssistantMessage(payload) {
...
export async function sendAssistantMessage(payload, handlers = {}) {
    const useRealStream = import.meta.env.VITE_USE_REAL_ASSISTANT_STREAM === 'true'
    const endpoint = import.meta.env.VITE_ASSISTANT_CHAT_URL || '/api/assistant/chat'

    if (!useRealStream) {
        return sendMockAssistantMessage(payload)
    }
    ...
    try {
        ...
        if (!response.ok || !contentType.includes('text/event-stream')) {
            throw new Error('Assistant stream endpoint is unavailable')
        }

        const streamed = await readSseStream(response, handlers)

        if (!streamed.content) {
            throw new Error('Assistant stream returned empty content')
        }

        return streamed
    } catch (error) {
        console.warn('助手流式接口不可用，已回退到本地回复', error)
        return sendMockAssistantMessage(payload)
    }
}
