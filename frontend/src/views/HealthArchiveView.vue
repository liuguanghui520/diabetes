...
function chooseDiagnosed(value) {
    archive.diagnosedDiabetes = value

    if (value === '否') {
        archive.diabetesType = ''
    }
}

function chooseDiabetesType(value) {
    archive.diabetesType = value
}
...
function saveHealthHistory(savedArchive) {
    ...
    const record = {
        ...
        diagnosedDiabetes: savedArchive.diagnosedDiabetes,
        diabetesType: savedArchive.diabetesType,
        waist: savedArchive.waist,
        sbp: savedArchive.sbp,
...
function saveArchive() {
    ...
    if (archive.diagnosedDiabetes === '是' && !archive.diabetesType) {
        ElMessage.warning('请选择糖尿病类型；不确定时可选择“类型未知”')
        return
    }

    const savedArchive = {
        ...archive,
        diabetesType: archive.diagnosedDiabetes === '是' ? archive.diabetesType : null,
...
<template>
    <div class="page archive-page">
        <header class="archive-header">
            <button class="back-button" aria-label="返回健康管理" @click="goBack">
                <el-icon>
                    <ArrowLeft />
                </el-icon>
            </button>
        ...
        <section class="form-card">
            ...
            <div class="form-field">
                ...
                <div class="option-grid two">
                    <button
                        :class="{ selected: archive.diagnosedDiabetes === '否' }"
                        @click="chooseDiagnosed('否')"
                    >
                        未患病
                    </button>

                    <button
                        :class="{ selected: archive.diagnosedDiabetes === '是' }"
                        @click="chooseDiagnosed('是')"
                    >
                        已患病
                    </button>
                </div>
            ...
            <div
                v-if="archive.diagnosedDiabetes === '是'"
                class="form-field"
            >
                <span>糖尿病类型</span>
                ...
                <div class="type-grid">
                    <button
                        ...
                        :key="item.value"
                        :class="{ selected: archive.diabetesType === item.value }"
                        @click="chooseDiabetesType(item.value)"
                    >
                        {{ item.label }}
                    </button>
                </div>
        ...
        <section class="form-card">
            ...
            <label class="upload-area">
                ...
                <small>支持图片或 PDF；解析结果需确认后才写入正式指标</small>
            </label>
        ...
        <section class="notice-card">
            ...
            <p>
                ...
                当前前端原型将数据保存于本浏览器本地存储；后续由 Express 写入 KingbaseES。
            </p>

            <label class="consent-row">
                <input v-model="archive.consentAccepted" type="checkbox">
                <span>我同意为健康档案、风险评估和生活方案生成处理相关健康数据</span>
            </label>
        </section>
